...
void game_render(Game *game) {
    ...
    al_set_target_bitmap(mapaExtra);
    al_clear_to_color( al_map_rgb(255,0,0) );


    al_set_target_bitmap(primerFrame);
    al_draw_bitmap(mapaExtra, 0,0,0);

    al_set_target_bitmap(algif_get_frame_bitmap(game->skeleton[game->dir], 1));
    al_draw_bitmap(mapaExtra, 10,0,0);

    al_set_target_bitmap(algif_get_frame_bitmap(game->skeleton[game->dir], 2));
    al_draw_bitmap(mapaExtra, 20,0,0);

    al_set_target_bitmap(algif_get_frame_bitmap(game->skeleton[game->dir], 3));
    al_draw_bitmap(mapaExtra, 30,0,0);
